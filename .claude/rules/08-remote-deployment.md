# 远程环境部署规则（DEPRECATED — 历史索引）

> **⚠️ 本文件已被 `08a-remote-analyzer-rules.md` + `08b-remote-builder-rules.md` + `08c-remote-validator-rules.md` 拆分替代。**
> **remote-env-setup Agent 已于 2026-06-03 弃用，职责拆为 analyzer 远程探测 / builder 远程部署 / validator 远程验证三段。**
> **本文件仅作历史索引保留。**

本文件原定义 remote-env-setup Agent 的全部操作规则。Agent 定义（`.claude/agents/remote-env-setup.md`）已加 deprecation banner，新工作流按三段 agent 划分。

**核心分工**：构建阶段由 builder agent（`project-manage-builder`）在本地完成，产出 `build/dev/` 部署包（含 node_modules、编译产物、Prisma 引擎）。Remote Build 阶段只负责将已构建的 dev/ 上传到远程、安装系统运行时、配置环境、启动服务。
...
- 缺失关键路径（如 workspace root 无 package.json）→ **终止部署**，提示用户先运行 builder agent 完成构建
...
builder agent（mode=remote）启动时**必须**确认本地 `build/tmp/` 目录存在且为空（可含 `.gitkeep` 占位）。
