...
deploy.md 包含 10 个章节。§2（目录结构）和 §3（部署步骤）由 builder agent 按实际 build/dev/ 写入；其余章节直接从 `version/<ver>/update_readme.md` 对应编号章节逐字复制，不修改内容。
...
builder agent 启动时**必须**确认本地 `build/tmp/` 目录存在且为空（可含 `.gitkeep` 占位）。
...
**注**：启动后端**仅在 remote 模式下作为部署后端服务由 builder 触发**（如需在本地构建阶段验证），实际环境启动验证由 validator 负责。
...
- 任何 FAIL 项 → builder **自闭环修复** → 重跑自检 → 通过后才算完成
- 禁止"标记 FAIL 但继续往下走"或"汇报主会话由主会话决定"
- 禁止向主会话报告"Build 已完成"但自检存在 FAIL 项
- 重跑后仍 FAIL → 视为构建失败，整个构建任务标 failed，汇报主会话请求介入
