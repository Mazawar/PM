...
**核心规则**：
- v2 schema 把状态拆为 `global`（项目级：Detect/Analyze/Build/Validate）/ `modules`（按模块：Plan/Generate/Execute/Report）/ `publishes`（历史）三段
...
2. **Analyze** — `project-manage-analyzer` agent 读仓库源码、推断技术栈/端口/中间件/凭据，写 `environment.json.analyzer.*` 段、生成 `playwright.config.ts`、初始化目录；如 `remoteConfig.server` 已绑定则做远程探测
3. **Build** — 主会话询问构建模式（local | remote）→ 写 `environment.json.build.mode` → 启动 `project-manage-builder` agent：local 编译+归档+组装 dev/；remote 在 local 基础上+安装远程运行时+上传+配置 .env+初始化 DB
4. **Validate** — 启动 `project-manage-validator` agent：执行 start.sh / 远程启动后端+Nginx → 健康检查 → 页面验证 → 登录验证 → 生成 seed.spec.ts → 写 SETUP.md。mode=remote 时询问用户确认新 baseURL 并同步 environment.json + playwright.config.ts
...
1. 接收任务 → 环境检查（三层检查：analyzer 缺失 → 启动 `project-manage-analyzer`；build 缺失 → 启动 `project-manage-builder`；validate 缺失 → 启动 `project-manage-validator`；三层都就绪才跳过）
2. **构建模式选择** — Analyze 完成后，使用 `AskUserQuestion` 询问用户"本地构建还是远程部署？"
   - **本地测试** → 写 `build.mode = "local"` → 启动 `project-manage-builder`（local 模式）
   - **远程部署** → 询问服务器 + deployPath → 写 `remoteConfig` + `build.mode = "remote"` → 启动 `project-manage-builder`（remote 模式）
     - Builder 安装系统运行时（Node.js/MySQL/Nginx 等），上传 dev/，配置 .env，初始化数据库
     - 完成后启动 `project-manage-validator` 做远程验证
   - **切换服务器** → 清空 `remoteConfig` + `analyzer.remoteProbe` + `build.mode = "remote"` → 启动 `project-manage-builder` 重跑
...
3. **不存在 analyzer 段**（未配置）→ 启动 `project-manage-analyzer` agent
   - Agent 分析源码、推断端口和凭据
   - 写 `environment.json.analyzer.*` 段、生成 `playwright.config.ts`
   - 运行 `init-dirs.mjs` 初始化目录
   - 如 `remoteConfig.server` 已绑定则做远程探测（按 08a）
   - 完成后通过 `updateStage('global', null, 'Analyze', { status: 'completed' })` 更新状态
4. **analyzer 已配置但 build 缺失** → 启动 `project-manage-builder` agent
   - 编译 → 归档 → 组装 `build/dev/`
   - 写 `start.sh`、`version-log.json`、执行 build/ 自检
   - mode=remote 时额外：打包 + 写 `deploy-config.json`/`nginx.conf` + 安装运行时 + 上传 + 配置 .env + 初始化 DB
   - 完成后通过 `updateStage('global', null, 'Build', { status: 'completed' })` 更新状态
5. **build 已完成但 validate 缺失** → 启动 `project-manage-validator` agent
   - 启动服务（local 执行 start.sh / remote 启动后端+Nginx）
   - 健康检查、页面验证、登录验证
   - 生成 `tests/seed.spec.ts` + `test-config/auth.json`、写 `SETUP.md`
   - mode=remote 时询问用户确认新 baseURL 并同步 environment.json + playwright.config.ts
   - 完成后通过 `updateStage('global', null, 'Validate', { status: 'completed' })` 更新状态
6. **三层都就绪** → 读取 `environment.json` 中的 `healthCheck`
7. 用 curl 检查服务是否在运行：`curl -s -o /dev/null -w "%{http_code}" <healthCheck.url>`
8. 检查结果：
   - **通过** → 继续测试流程
   - **未通过** → 启动 `project-manage-validator` agent，由 Agent 负责启动服务并验证

### 远程部署前置检查（当用户要求远程部署时追加）

在启动 `project-manage-builder`（mode=remote）之前，额外检查 `build/dev/` 是否存在且结构完整：
...
**`build/dev/` 不完整 → 跳过 builder，由主会话先启动 `project-manage-analyzer`（如未跑）→ 启动 `project-manage-builder` 执行生产构建。主会话禁止自行编译打包。**
...
测试执行管线（环境检查由 Analyze/Build/Validate 三段按需触发，不在此管线中）：
...
| 阶段 | 确认内容 |
|------|---------|
| Build+Validate 后（mode=remote） | 确认 environment.json 的 baseURL 变更（远程 IP） |
| Analyze 后 | 确认 `environment.json.analyzer.*` 字段完整性（端口/技术栈/凭据） |
| Build 前 | 确认构建模式（local / remote） |
...
- `project-manage-analyzer` / `project-manage-builder` / `project-manage-validator` 创建目录时，若上述文件/目录已存在必须保留原内容
