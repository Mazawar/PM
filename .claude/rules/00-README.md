...
├── 04-builder-rules        builder agent — 本地构建 + 远程部署（生产包组装+SSH 部署）
├── 05-validator-rules      validator agent — 启动验证 + 远程验证 + 页面验证 + SETUP.md（附录：runner 工具）

测试执行层（Agent 约束，管线下游）
├── 06-planner-rules        planner agent — TC 编号、计划分层、用户案例优先级、用户确认流程
...
| Build | 01, 02, 04 | project-manage-builder |
...
| `project-manage-builder.md` | `04-builder-rules.md` |
