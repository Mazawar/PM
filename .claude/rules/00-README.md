...
├── 03-setup-environment    Setup Agent — 数据库初始化、端口推断、脚本验证、问题处理（索引）
...
| Agent 定义 | 约束规则 |
|-----------|---------|
| `project-manage-setup.md`（**DEPRECATED**） | `03-setup-environment.md`（索引） |
...
| `remote-env-setup.md`（**DEPRECATED**） | `08-remote-deployment.md`（索引） |
