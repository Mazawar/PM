...
- **Setup** 仅首次测试触发，由 `project-manage-setup` agent 自动分析源码、推断端口、生成环境配置
...
| 阶段 | Agent | 职责 | 输出 |
|------|-------|------|------|
| 配置 | **project-manage-setup** | 首次测试时分析源码、推断端口、生成环境配置 | `playwright.config.ts`、`environment.json`、`start.sh` |
