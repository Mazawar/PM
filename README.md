...
- **可中断恢复** — 管线状态持久化（global 项目级 + modules 模块级 + publishes 历史），崩溃或中断后从断点继续；多模块进度独立
...
- **Plan / Generate / Execute / Report** — 模块级阶段（`modules.<name>`），按模块独立追踪，互不覆盖
...
- **管线可中断恢复** — `.pipeline-state.json` 记录状态（global 项目级 + modules 模块级 + publishes 历史），新会话从断点继续；多模块进度独立，互不覆盖
...
定时扫描通过 `CronCreate` 配置，含自动续签机制（7 天过期前自动重建）。
...
项目规则定义在 `.claude/rules/` 下（自动加载），Agent 详见 `docs/agents.md`。
