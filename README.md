...
- **Analyze / Build / Validate** — 项目级阶段：analyzer 分析源码 → builder 生产构建 → validator 启动验证
...
│   │   ├── project-manage-builder.md    # Builder（生产构建+部署）
...
| 构建 | **builder** | 生产构建、归档、组装 dev/、远程部署（SSH） | `build/` 下全部产物、`start.sh` |
| 验证 | **validator** | 启动服务、健康检查、页面/登录验证 | `SETUP.md`、`seed.spec.ts`、`auth.json` |
...
| 04 | `04-builder-rules.md` | builder：生产构建 + 远程部署（SSH/Nginx/DB 初始化） |
| 05 | `05-validator-rules.md` | validator：启动验证 + 远程验证 + runner 工具 |
| 06 | `06-planner-rules.md` | planner：TC 编号、计划分层、用户案例优先级 |
